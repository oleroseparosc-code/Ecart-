...
- `약제팀 라벨/원내보유의약품리스트.xlsx`
  ...
  - The label studio derives its drug list and label defaults only from this generated source and the `라벨 생성규칙` sheet dimensions. Its three-column `약품 마스터` writes shared caution/storage/management flags back to the workbook while keeping dosage-form and attachment-label settings pharmacy-only; shared fields overlay ward, narcotic, and E-cart label output by `약품코드`.
