...
- `약제팀 라벨/원내보유의약품리스트.xlsx`
  ...
  - Every saved pharmacy master row, including edits to existing drugs, is kept in the shared `pharmacyAdditionalRows` overlay and synchronized by drug code. A new non-controlled pharmacy row is projected into the administrator's stock-assignment candidates with its shared storage and warning flags (including `용량확인`); controlled rows remain on the narcotic pathway. The cloud sync service persists app state in D1 and the latest pharmacy workbook in R2, so pharmacy editor, admin, and viewer clients poll and merge the same changes without opening a local file picker. Final pharmacy label saves also write the selected `최종 라벨 크기`, complete draft, and injectable-anticancer `용해액`/`용해액 2` columns; reopening that drug restores the saved size and both diluent lines as defaults.
