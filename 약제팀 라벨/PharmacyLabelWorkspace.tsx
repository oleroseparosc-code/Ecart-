...
import {
  ...
  isAnticancerCategory, isInjectableAnticancerCategory,
...
export function PharmacyLabelWorkspace({ rows, savedLabels, isLoading, onBack, onSaveLabel, onSaveLabels, onPrint, onHospitalDrugWorkbookUpload, onSaveMaster, onSaveManyMaster, onDeleteMaster, onBulkSaveMaster, onBulkDeleteMaster, standalone = false }: Props) {
  ...
  const isAnticancerLabel = isAnticancerCategory(displayCategory);
    ...
    {activeTab === "master"
      ...
      <section className="pharmacy-label-canvas-panel">
        ...
        <div className="pharmacy-label-canvas">{draft ? <article className={`pharmacy-print-label label-size-${draft.size.presetKey} ${isAnticancerLabel ? "anticancer" : ""} ${displayCategory === "마약/향정" ? "controlled-drug-label" : ""} ${displayCategory === "고가약" ? "high-cost" : ""} ${hasCustomOuterBorderColor ? "custom-outer-border" : ""} ${hasCustomGeneralFluidTextColor ? "custom-general-fluid-text-color" : ""} ${storageOnlyClass} ${storageToneClass} ${isCapLabel ? "cap-label" : ""} ${isColoredCapLabel ? "colored-cap-label" : ""} ${isSideLabel ? "side-label" : ""} ${isExternalShelfLabel ? "external-shelf-label" : ""} ${displayCategory === "시럽" ? "syrup-label" : ""} ${displayCategory === "영양수액" ? "nutrition-fluid-label" : ""} ${isGeneralFluidLabel ? `general-fluid-label fluid-tone-${generalFluidTone}` : ""} ${isInjectionLabel ? "injection-label" : ""} ${isHeparinLabel ? "heparin-label" : ""} ${isAmpouleHolder ? "has-ampoule-holder" : ""} ${!showTopBanner ? "no-top-banner no-warning" : ""}`} style={labelStyle}>
          ...
          {!isExternalShelfLabel && (draft.printable.footer.enabled || draft.warnings.includes("위해의약품")) && <footer className={isAnticancerLabel ? `anticancer-footer ${hasSecondReconstitution ? "has-second-reconstitution" : ""}` : isHeparinLabel ? "heparin-footer" : ""}>{draft.warnings.includes("위해의약품") ? "<캅셀개봉. 분쇄 금지>" : isAnticancerLabel ? <><span className="anticancer-footer-title">항암제</span>{draft.printable.reconstitution && (hasSecondReconstitution ? <span className="anticancer-footer-reconstitution"><b>용해액:</b><span className="anticancer-footer-reconstitution-lines"><span>{draft.printable.reconstitution}</span><span>{draft.printable.reconstitutionSecondary}</span></span></span> : <span className="anticancer-footer-reconstitution">용해액: {draft.printable.reconstitution}</span>)}</> : draft.printable.footer.text}</footer>}
          </>}
      ...
      <aside className="pharmacy-tool-panel">
        ...
        <details open><summary>표시 내용</summary><div className="pharmacy-tool-body">
          ...
          <label>한글약품명<input value={draft?.printable.koreanName ?? ""} onChange={(e) => draft && patch({ printable: { ...draft.printable, koreanName: e.target.value } })}/></label><label>용량<input value={draft?.printable.strength ?? ""} onChange={(e) => draft && patch({ printable: { ...draft.printable, strength: e.target.value } })}/></label><label>약품 위치<input value={draft?.location ?? ""} onChange={(e) => patch({ location: e.target.value })}/></label><label>ATC 번호<input value={draft?.atc ?? ""} onChange={(e) => patch({ atc: e.target.value })}/></label>{isInjectableAnticancerCategory(displayCategory) && <><label>용해액 1<input value={draft?.printable.reconstitution ?? ""} onChange={(e) => draft && patch({ printable: { ...draft.printable, reconstitution: e.target.value } })}/><span className="pharmacy-diluent-quick-options">빠른 선택 {(["NS", "D5W", "WI"] as const).map((value) => <button type="button" key={value} className={draft?.printable.reconstitution === value ? "active" : ""} onClick={() => draft && patch({ printable: { ...draft.printable, reconstitution: value } })}>{value}</button>)}</span></label><label>용해액 2<input value={draft?.printable.reconstitutionSecondary ?? ""} onChange={(e) => draft && patch({ printable: { ...draft.printable, reconstitutionSecondary: e.target.value } })}/></label></>}
        </div></details>
      </aside>
      </>}
    </section>}
